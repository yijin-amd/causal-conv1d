...
int main() {
    ...
    // Test cases matching CUDA version (test_cuda_fwd_kernels.cu)
    // Note: bias is always true (consistent with CUDA version)
    // Note: width=2 and width=3 test cases are temporarily commented out (not supported yet)
    TestCase test_cases[] = {
        // ========== dim=64 测试 (小规模, 覆盖所有width和seqlen范围) ==========
        {"dim64-seq128-w4", 2, 64, 128, 4, true, false},   // 极小
        {"dim64-seq512-w4", 2, 64, 512, 4, true, false},   // 小
        {"dim64-seq512-w4-silu", 2, 64, 512, 4, true, true},    // 小+SiLU
        {"dim64-seq1024-w4", 2, 64, 1024, 4, true, false},  // 中
        {"dim64-seq1024-w4-silu", 2, 64, 1024, 4, true, true},   // 中+SiLU
        // {"dim64-seq2048-w2", 2, 64, 2048, 2, true, false},  // 中大+width=2 [暂不支持width=2]
        // {"dim64-seq2048-w3", 2, 64, 2048, 3, true, false},  // 中大+width=3 [暂不支持width=3]
        {"dim64-seq2048-w4", 2, 64, 2048, 4, true, false},  // 中大+width=4
        
        // ========== dim=256 测试 (中等规模, 不同seqlen+width) ==========
        {"dim256-seq256-w4", 4, 256, 256, 4, true, false},  // 短seq
        {"dim256-seq512-w4", 4, 256, 512, 4, true, false},  // 中短seq
        {"dim256-seq512-w4-silu", 4, 256, 512, 4, true, true},   // 中短seq+SiLU
        {"dim256-seq1024-w4", 4, 256, 1024, 4, true, false}, // 中seq
        {"dim256-seq1024-w4-silu", 4, 256, 1024, 4, true, true},  // 中seq+SiLU
        // {"dim256-seq2048-w2", 4, 256, 2048, 2, true, false}, // 长seq+width=2 [暂不支持width=2]
        // {"dim256-seq2048-w3", 4, 256, 2048, 3, true, false}, // 长seq+width=3 [暂不支持width=3]
        {"dim256-seq2048-w4", 4, 256, 2048, 4, true, false}, // 长seq+width=4
        
        // ========== dim=512 测试 (中大规模, 关键性能区) ==========
        {"dim512-seq512-w4", 4, 512, 512, 4, true, false},  // 短seq
        {"dim512-seq1024-w4", 4, 512, 1024, 4, true, false}, // 中seq
        {"dim512-seq1024-w4-silu", 4, 512, 1024, 4, true, true},  // 中seq+SiLU
        {"dim512-seq2048-w4", 4, 512, 2048, 4, true, false}, // 长seq (峰值性能点)
        {"dim512-seq2048-w4-silu", 4, 512, 2048, 4, true, true},  // 长seq+SiLU
        // {"dim512-seq2048-w2", 4, 512, 2048, 2, true, false}, // 长seq+width=2 [暂不支持width=2]
        // {"dim512-seq2048-w3-silu", 4, 512, 2048, 3, true, true},  // 长seq+width=3+SiLU [暂不支持width=3]
        
        // ========== dim=1024 测试 (大规模) ==========
        {"dim1024-seq512-w4", 8, 1024, 512, 4, true, false},  // 短seq
        {"dim1024-seq1024-w4", 8, 1024, 1024, 4, true, false}, // 中seq
        {"dim1024-seq1024-w4-silu", 8, 1024, 1024, 4, true, true},  // 中seq+SiLU
        {"dim1024-seq2048-w4", 8, 1024, 2048, 4, true, false}, // 长seq
        {"dim1024-seq2048-w4-silu", 8, 1024, 2048, 4, true, true},  // 长seq+SiLU
        {"dim1024-seq4096-w4", 8, 1024, 4096, 4, true, false}, // 超长seq
        
        // ========== dim=2048 测试 (超大规模) ==========
        {"dim2048-seq1024-w4", 8, 2048, 1024, 4, true, false}, // 中seq
        {"dim2048-seq2048-w4", 8, 2048, 2048, 4, true, false}, // 长seq
        {"dim2048-seq2048-w4-silu", 8, 2048, 2048, 4, true, true},  // 长seq+SiLU
        {"dim2048-seq4096-w4", 8, 2048, 4096, 4, true, false}, // 超长seq
        // {"dim2048-seq2048-w2", 8, 2048, 2048, 2, true, false}, // 长seq+width=2 [暂不支持width=2]
        // {"dim2048-seq2048-w3-silu", 8, 2048, 2048, 3, true, true},  // 长seq+width=3+SiLU [暂不支持width=3]
    };
