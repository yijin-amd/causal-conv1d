...
# Standalone test script for causal_conv1d_channellast_fwd_kernel
set -e

cd "$(dirname "$0")"

echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Causal Conv1D Channel-Last Kernel Test                       ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""

# Parse arguments
TEST_MODE=0  # 0=all, 1=accuracy, 2=performance
...
hipcc $HIPCC_FLAGS test_channellast_kernel.cpp -o test_channellast_kernel 2>&1 | tee compile.log
...
./test_channellast_kernel $TEST_MODE
